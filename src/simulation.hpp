...
public:
  ...
  void initialize(double dt, double temperature, double gamma, double mass,
                  int nsteps, int snapshot, int rngseed) {
...
// Start the timer
    ...
    while (ncur < n) {
      ...
      // Report progress
      if (ncur % (n / 10) == 0) {
        std::cout << "  " << (int)(100. * (double)ncur / (double)n)
                  << "% complete" << std::endl;
      }
    }
...
// Report timing
    ...
    run_time = (std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                1E6);
    std::cout << std::endl
              << "Total time: " << std::setprecision(5) << run_time
              << " seconds" << std::endl;
